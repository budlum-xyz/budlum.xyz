...
## Latest exact-geometry status — 2026-07-24

The exact VECTOR geometry backlog has been resolved offline from the checked-in OpenFig `.fig` binary after explicit user approval.

Current state:

- `figma-audit/missing-exact-assets.json`: `0` entries
- `figma-audit/remaining-exact-assets-plan.json`: `0` frames / `0` records
- `figma-audit/render-coverage-summary.json`: `0` renderable vector-like nodes skipped for missing geometry
- `figma-audit/openfig-geometry-final-report.md`: cumulative OpenFig recovery report

The historical live-refresh batches below are retained only as rate-limit guidance for future Figma API work. They are no longer the active geometry backlog. Do not rerun old exact-geometry batches unless `npm run figma:plan` generates new missing records.
...
## Historical recommended order for remaining exact vector refresh — obsolete after OpenFig resolve
...
Historical missing exact vector frames from `figma-audit/missing-exact-assets.json` before the OpenFig resolve:
...
Historical first batch after cooldown (do not run unless regenerated by `npm run figma:plan`):
...
Historical second batch (do not run unless regenerated by `npm run figma:plan`):
...
Historically largest frames left for later:
